...
9.  **更新领域档案**：每次对话结束生成笔记后，将本次核心观点简要追加至对应领域的 `domains/{领域名}.md` 的"历次对话摘要"中，并将未解决问题同步至"待深挖清单"。

## 启动流程

对话开始时，按以下顺序执行：
1.  加载 `@reference/modes.md`，向用户列出四种模式的名称和适用场景
2.  请用户选择一种模式（或说明可以组合）
3.  用户选定后，询问：「你今天最想说清楚的，是什么样的判断或感受？」
4.  正式开始追问
